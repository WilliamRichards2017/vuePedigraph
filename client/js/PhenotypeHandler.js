...
export default class PhenotypeHandler {
  ...
  replacePTCIDs() {
    ...
    for (let key in self.asparagus) {
      ...
      if(self.linkIdMap.hasOwnProperty(key)) {
         let linkKey = self.linkIdMap[key].toString();
         if(self.idMap.hasOwnProperty(linkKey)) {
           newKey = self.idMap[linkKey].split('-')[1].toString();
         }
      }
